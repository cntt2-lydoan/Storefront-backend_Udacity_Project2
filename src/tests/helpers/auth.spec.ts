...
fdescribe('Auth ', () => {
  ...
  fit('should login with wrong password to result is error', async () => {
  ...
  fit('should login success to result is token', async () => {
    const response = await request
      .post(`/login/`)
  ...
  fit('should authenticate info user', async () => {
    const response = await request
      .post(`/authenticate/`)
      .send({ username: 'username', password: 123 });
  ...
  fit('should login with invalid user to result is error', async () => {
    const response = await request
      .post(`/login/`)
